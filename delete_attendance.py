...
MASTER_ID = 213989027376  # Replace with master card ID
...
def delete():
    try:
        ...
        cursor.execute(
            f"SELECT attended, max_classes FROM {subject} WHERE srn = ?", (srn,))
        ...
        if result:
            ...
            if attended > 0:
                cursor.execute(
                    f"UPDATE {subject} SET attended = ? WHERE srn = ?", (attended - 1, srn))
                conn.commit()
